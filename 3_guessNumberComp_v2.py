...
def computer_guess(x):
	...
	while feedback != 'c' and low != high:
		...
		if feedback == 'h':
			high = guess - 1
			print(f'{high} <- this is what happend to variable high. \
		this is what happened to variable guess -> {guess}')
		elif feedback == 'l':
			low = guess + 1
			print(f'"{low}" <- this is varible low. -> this is variable guess "{guess}"')
